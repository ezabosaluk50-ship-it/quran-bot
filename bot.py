...
ADMIN_ID = 5410915902  # استبدله بـ ID حسابك لرؤية الإحصائيات الحقيقية
...
def build_readers_keyboard():
    ...
    for i, (name, _) in enumerate(READERS_LIST):
        ...
        if len(row) == 3: # تم إصلاح النقطتين هنا
            keyboard.append(row)
            row = []
    if row: keyboard.append(row)
    return InlineKeyboardMarkup(keyboard)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["ابدأ 🔄", "/start"]:
        ...
        if update.effective_user.id == ADMIN_ID:
            await update.message.reply_text(f"📊 عدد المستخدمين الحقيقي: {count}")
        else:
            await update.message.reply_text(f"📊 عدد المستمعين عبر البوت: {count + 50}")
